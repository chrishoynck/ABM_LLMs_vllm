...
| `optimized_phq9_human/` | TextGrad PHQ-9 prompt runs re-optimized on the human-optimized corpus (train `finetune/train_posts.csv`, test `finetune/test_posts.csv`, minimal start prompt; seeds 23,24,25,32,33; `run_meta.txt` per run) — the LLM counterpart of `bert_regression_finetuned/` | paper assessment tables (planned replacement of the base-set rows) |
...
| `bert_regression/eval_baseline_<tag>/`, `bert_regression_finetuned/eval_<tag>300/`, `bert_regression_finetuned_<tag>/` | multi-model arm (tags `gemma4`, `mistral`): Qwen-trained regressors evaluated on / fine-tuned on that generator's posts (`run_finetune.sh` with `GEN_TAG`) | paper multi-model table |
