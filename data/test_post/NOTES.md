...
| `bert_regression/eval_baseline_<tag>/`, `bert_regression_finetuned/eval_<tag>300/`, `bert_regression_finetuned_<tag>/` | multi-model arm (tag = `gemma4`, `kimi`): Qwen-trained regressors evaluated on / fine-tuned on that generator's posts (`run_finetune.sh` with `GEN_TAG`) | paper multi-model table |
