...
and `--model mistral-small-24b` (`loaders.MODEL_ALIASES`), each with its own
decoding pair in `loaders.STUDENT_DECODING` (vendor-anchored relative to Qwen's
0.7/0.9), both run in `.venv_vllm_g4`; the same Qwen human-optimized prompt is
used for all. A Kimi-Linear arm was run and dropped (see `data/README.md`
section 4a).
