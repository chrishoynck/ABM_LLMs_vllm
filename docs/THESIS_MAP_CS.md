...
| Results §Cognitive Distortions + App. C §CDS Lexicon | `checks/check_cds_tracks_phq9.py` (detector: `src/utils/tools/cds.py`) → `plots/cds_validation.png` | `PYTHONPATH=src python checks/check_cds_tracks_phq9.py` | `data/distorted_language_ngrams.tsv`, `data/finetune/cds_by_phq9*.csv` |
| App. C §Classification vs. Embedding Space | `src/utils/tools/plot_assessment_diagnostics.py` (`all`) | hand-run — see `../src/README.md` ("Hand-run CLIs") | `data/assessors/bert/teacher/test_blocks_seed35.csv` |
