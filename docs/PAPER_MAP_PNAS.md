...
| Fig `syntetic_data_CDS.png` (§2.3) | `plots/cds_validation.png` | `PYTHONPATH=src python checks/check_cds_tracks_phq9.py` | `data/finetune/cds_by_phq9*.csv`, `data/distorted_language_ngrams.tsv` |
...
| Tables 1–2 (estimators / shift) | aggregate CSVs + figs | `bash scripts/assessment/run_eval_comparison.sh` → `utils.visualization` | `data/test_post/method_comparison/`, `data/assessors/bert/teacher/eval/on_qwen27_optimized/aggregate.csv`, `data/test_post/optimized_phq9/*/eval_on_*` |
