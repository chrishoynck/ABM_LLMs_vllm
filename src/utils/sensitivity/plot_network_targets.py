...
Config labels name the metric each one probes (e.g. "low C", "goal C", "high
PHQ-9"), not the degree — degree is its own panel now, and it varies between
configs that share a label (read it off panel (d)).
...
_K6_RUNS    = {"goal C": "old_debiased/2_1655_d6_dim5"}   # saved k=6 run (C in band; renamed from debiased/)
...
def plot_network_targets(out: str) -> None:
    # (label, points). Labels name the metric the config probes, not its degree —
    # degree is panel (d) and varies between configs sharing a label. Duplicate
    # labels are intentional (e.g. two "low C" at different degrees).
    configs = [
        ("low C",  _lowk_points(_LOWK_COMBO_LOW_C, degree=_LOWK_DEGREE_3)),  # k=3
        ("low C",  _lowk_points(_LOWK_COMBO_LOW_C)),                         # k=4.5
        ("goal C", _lowk_points(_LOWK_COMBO)),                              # k=4.5
    ]
    for label, run in _K6_RUNS.items():
        configs.append((label, _k6_points(run)))                            # goal C, k=6
    configs.append(("high\nPHQ-9",
    ...
    # at degree 6.
    directed = [
        _lowk_directed_points(_LOWK_COMBO_LOW_C, degree=_LOWK_DEGREE_3),  # low C, k=3
        _lowk_directed_points(_LOWK_COMBO_LOW_C),                         # low C, k=4.5
        _lowk_directed_points(_LOWK_COMBO),                              # goal C, k=4.5
        _lowk_directed_points(_LOWK_COMBO, degree=6.0),                  # goal C, k=6 (rebuilt)
