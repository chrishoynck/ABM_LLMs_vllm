"""Surface-cue ablation for the PHQ-9 regressor.

Re-scores a tweets_with_phq9 CSV after removing emoji, expressive punctuation
(!, ?, ellipsis, hashtags) or both, and reports how much the regressor's MAE and
correlation change. A large drop means the regressor leans on surface cues; a
small drop means it reads the words.

Usage:
    PYTHONPATH=src python -m utils.analyses.surface_ablation \
        --regressor data/test_post/bert_regression/Qwen3.5-27B_seed34/regressor.pt \
        --csv data/finetune/test_posts.csv --out-dir data/test_post/surface_ablation
"""
...
def main():
    ...
    ap.add_argument("--regressor", required=True)
    ...
    for name, fn in VARIANTS.items():
        d = src.copy(); d["tweet"] = d.tweet.map(fn)
        csv_path = os.path.join(a.out_dir, f"{name}.csv"); d.to_csv(csv_path, index=False)
