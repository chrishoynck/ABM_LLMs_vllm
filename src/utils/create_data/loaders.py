"""Shared loaders for the create_data CLIs: model aliases, vLLM and tokenizer setup, persona and neighbour-pool loaders.

Keeps the decoding and dtype defaults in one place (`MODEL_ALIASES`, `STUDENT_DECODING`,
`get_llm`, `get_tokenizer`), loads the (persona, PHQ-9) eval file and the
optimizer-aligned (instruction, format block, prompts) tuple, and gathers the neighbour
pool the same way `prompt_optimizer` does.
"""
...
def resolve_student_decoding(model_id: str, temp: float | None = None,
    ...
    `STUDENT_DECODING` is used. Unknown models fall back to the Qwen
...
def gather_neighbor_pool(roots: list[str] | tuple[str, ...] | None = None,
    ...
    Same file walk as `prompt_optimizer._generate_file_path`; the pool feeds straight
    into `TestLLMs.neighbor_pool`.
    """
